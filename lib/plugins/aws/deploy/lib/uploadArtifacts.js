...
module.exports = {
  ...
  async uploadZipFile(artifactFilePath) {
    ...
    const hash = await getHashForFilePath(artifactFilePath);
    const hexHash = Buffer.from(hash, 'base64').toString('hex');

    const s3Key = `${this.serverless.service.package.artifactDirectoryName}/${fileName}/${hexHash}`;

    if (await this.isAlreadyUploaded(s3Key)) {
      this.serverless.cli.log(`Artifact ${s3Key} already uploaded`);
      return;
    }
    this.serverless.cli.log(`Uploading service ${fileName} file to S3...`);
